...
class SimpleCarAgent(Agent):
    ...
    def receive_feedback(self, reward, train_every=20, reward_depth=7):
        """
        ...
        if not self.evaluate_mode and (len(self.reward_history) >= train_every) and not (self.step % train_every):
            ...
            self.neural_net.SGD(training_data=train_data, epochs=20, mini_batch_size=train_every, eta=0.1)
